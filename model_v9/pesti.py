...
def degrade(model, layer,
    ...
    conc_aq_heavy_ini = fractions["heavy"]  # negative values!!
    ...
    if DEBUG_pest:
        if layer == 0:
            model.report(dt_50, 'dt50')
            model.report(k_b, 'kbdeg')
            model.report(conc_layer_aq, 'totCaq')
    ...
    if DEBUG_pest:
        if layer == 0:
            ...
            model.report(mass_heavy_fin, "hMfin")
...
def getLatMassDeltaFlux(model, layer, theta_sat, theta_fcap,
    ...
    wat_loss = max((c * (depth * theta_layer - depth * theta_fcap)), scalar(0))
    wat_gain = rel_wetness * accuflux(model.ldd_subs, wat_loss)
    ...
    massdC_loss = mass_loss*delta_layer
    ...
    return {
        'wat_loss': wat_loss,  # mg
        'wat_gain': wat_gain,  # mg
...
def getVolatileMass_MB(model, app_days, temp_air, theta_sat, rel_diff_model="option-1", sorption_model="linear", gas=True):
