...
The run-off equation is given by [@Neitsch2009]:
...
Deep percolation (DP) is assumed to be negligible at moisture levels below field capacity. Above this moisture level percolation is given by Raes \textit{et al.}, [@Raes:2002}:
...
Later flow occurs when the soil moisture content exceeds the field capacity and is represented as the difference between inflow and outflow of a downstream cell $i$ at soil layer $z$ according to Manfreda \textit{et al.}, [@Manfreda2005]:

\begin{equation}
\Delta LF_{i,z} = \Big( \frac{W_{i,z} \sum^{N(t)}_{j=1}max[c_{j,z}(s_{j,z}-s_{fc,j,z}),~0] }{ \sum^{N(t)}_{j=1} W_{j,z} } \Big) - max[c_z(s_z-s_{fc,z}),~0]
\label{eq:LF}  
\end{equation}

were $c$ is the subsurface flow coefficient ($\approx$ 0.25 $d^{-1}$, if $dt$ $\leq$ $1~d$, [@Manfreda2005]), $s$ and $s_{fc}$ are the soil moisture and the field capacity of the soil profile [mm], respectively. $N(t)$ is the number of cells exceeding field capacity and at steepest slope to cell $i$.  $W_{(i,k)}$ and $W_{(j,k)}$  are the topographic wetness indices (m) at cell $i$ (downstream cell) and $j$ (upstream cell(s)), respectively. The wetness index describes the tendency for water to accumulate spatially in regions characterized by a relatively low local slope and a large upstream drainage area (cite: Kirby 1975). It is given by (cite Kirby 1975  or Beven and Kirk 1979):

\begin{equation}
W_j =ln\Big(\frac{a}{tan \beta} \Big)
\label{eq:Wz}  
\end{equation}

were $a$ is the drainage area $(m^2)$ from upstream cells defined by the Local Drain Direction (LDD) network and tan $\beta$ is the local slope in radians derived from the Digital Elevation Model (DEM).
...
where $RD$ (m) is the total or individual soil layer's rooting depth and $RD_{z, 0.5}$ is the soil depth at the middle of the root extension for layer $z$.    
...
The concentration of pesticide within the soil system ($mg/L$) along each layer $z$ is given as a function of its partitioning across the soil's gaseous, aqueous and adsorbed phases by:
...
where $\theta_{gas_z}(t) = \theta_{sat_z}(t) - \theta_{z}(t)$, $C_{gas_z}$ is the concentration in the gas phase ($mg/L$), $C_{aq_z}$ the concentration in aqueous phase ($mg/L$) and $C_{ads_z}$ the concentration in adsorbed phase ($mg/g$) with the bulk soil density $\rho_{b_z}$ in $g/L$.

Converting to mass base don the cell area ($A_i$, $m^2$) and model layer depth ($D_z$, $mm$):
...
The partition of pesticide concentrations into the dissolved $C_{aq}$ ($mg/m^3$) and adsorbed phases $C_{ads}$ (mg/Kg) is determined by the pesticide dissociation coefficient $K_d$ (L/Kg):
...
where $K_{oc}$ is the pesticide octanol partition coefficient (mL/g) and $f_{oc}$ is the fraction of organic carbon (-) in soil. 


The dissolved concentration ($mg/L$) is given by [@Whelan1987]:
...
where the pesticide mass $M_p$ ($mg/m^2$) is assumed to be perfectly mixed within the soil profile's water content $\theta$ (-) of a soil layer $z$ with depth $D$ (mm). 
...
where $\rho_{b_z}$ is bulk density (Kg m$^{-3}$) and pesticide dissociation coefficient $K_d$ (mL/g).
...
Substituting eq. \ref{eq:conc_gas_top} into eq. \ref{eq:Jva} yields the mass flux lost to the atmosphere ($mg/m^2d$):
...
where the Effective Depth of Interaction (EDI) refers to the mixing layer depth (mm), $\theta$ is soil moisture $(m^3/m^3)$, RO is run-off (mm) and $C_m$ is concentration in the mixing layer (mg/mm$^3$). The paramater $b$ is a calibration constant (assuming, $1 \ge b > 0$) and where $z$ is the depth of the simulated top-soil layer. In this model, $\beta$ accounts for an exponential decrease in the ability of overland flow to mix with soil water as depth increases. 
...
```{r include=FALSE}
# \textit{Mixing layer}:
# Omitted the Mixing layer part as this is redundant considering that the soil layer is updated based on the transfer to overland flow.
#\begin{equation}
#n\frac{\partial (z C_m) }{\partial t} = K_L (C_{runoff} - C_m)
#\label{eq:d-mlm-ml}
#\end{equation} 
# where $n$ is the soil porosity (-), $z$ is the soil depth (mm), considered here as the depth of the mixing layer. 

# Omitting the sorption parameter (will be dealt seprately and thus implicit in C_m)
#\begin{equation}
#S = \rho_b K_d(1-n)\frac{\partial (C_m) }{\partial t}
#\label{eq:d-mlm-sorption}
#\end{equation} 
```
...
Later mass flux is proportional to the aqueous concentration in lateral water flow. The net mass flux is given by adapting eq. \ref{eq:LF}:
...
### Photodegradation (isotopes!!!)

...to formalize and document.
...
Temperture is an important parameter regulating chemical and biological processes. To simulate its evolution across time and space this model follows [@Neitsch2009], where the average daily soil temperature ($^{\circ}$C) at the center of each soil layer $k$ is given by:
...
where $l$ is a lag coefficient (0 - 1.0) regulating the influence of the previous day's temperature, the subscript $t-1$, refers to the previous day, $\overline{T}_{AAir}$ is the average annual air temperature and $T_{ssurf}$ is the soil surface temperature at time $t$. The depth factor $df$ is given by,
...
where $zd$ is the ratio of the depth at the center of the soil layer $D_{k_{mid}}$ (mm) to the damping depth $dd$ (mm) such that:
\begin{equation}
...
where $\rho_b$ is the soil bulk density (mg m$^{-3}$) and SW is water content (mm $H_2O$) in the soil profile $D_k$ (mm). 

The soil surface temperature $T_{ssurf}$ in eq. \refeq{tempSoil}, is a fucntion of the previous day's soil temperature, amount of ground cover captured by a crop factor $bcv$ and the temperature of bare soil $T_{bare}$, such that:
