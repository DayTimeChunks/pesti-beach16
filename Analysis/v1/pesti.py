...
def get_conc_aq(model, layer, theta_sat, sorption_model="linear", gas=True):
    ...
    if gas:
        ...
        conc_layer_aq = mass_layer / ((model.cell_area * depth) *
                                      (theta_gas * model.k_h + theta_layer * retard_layer))  # mg/L
    else:
        # Whelan, 1987 # No gas phase considered
        conc_layer_aq = (mass_layer / model.cell_area) / (theta_layer * retard_layer * depth)  # mg/L
...
def get_mass_from_aq(model, layer,
    ...
    if gas:
        if sorption_model == "freundlich":
            ...
            mass_layer = model.cell_area * depth * (theta_gas * model.k_h * conc_aq  # gas
        ...
        # Whelan, 1987 # No gas phase considered
        mass_layer = model.cell_area * depth * (theta_aq_layer * conc_aq
...
def get_mass_from_ads(model, layer,
    ...
    if gas:
        ...
        if sorption_model == "freundlich":
            print("Freundlich not implemented, running Linear Sorption")
            # Leistra et al., 2001
            mass_layer = model.cell_area * depth * (theta_gas * model.k_h / model.k_d * conc_ads  # gas
        ...
        print("Running Linear Sorption, no Gas Phase")
        # Whelan, 1987 # No gas phase considered
        mass_layer = model.cell_area * depth * (theta_aq_layer * conc_ads / model.k_d
...
def getVolatileMass(model, app_days, temp_air, theta_sat, rel_diff_model="option-1", sorption_model="linear", gas=True):
    ...
    if model.jd_cum in app_days:
        ...
        volat_flux = (conc_gas_layer / (r_a + r_s)) * model.cell_area  # mg/day
    else:
        volat_flux = 0
...
def getRunOffMass(model, theta_sat, precip, runoff_mm,
                  ...
                  gas=True):
    depth, theta_layer = model.z0, model.theta_z0
    ...
    if gas:
        ...
        conc_layer_aq = model.pestmass_z0 / ((model.cell_area * depth) *
                                             (theta_gas * model.k_h + theta_layer * retard_layer))  # mg/L
    else:
        # Whelan, 1987 # No gas phase considered
        conc_layer_aq = (model.pestmass_z0 / model.cell_area) / (theta_layer * retard_layer * depth)  # mg/L

    if transfer_model == "simple-mt":
        mass_ro = conc_layer_aq * runoff_mm * model.cell_area  # mg
        ...
        mass_ro = (runoff_mm * model.cell_area) * exp(-b * model.z0) * conc_layer_aq  # mg
    elif transfer_model == "nu-mlm":
        # non-uniform-mixing-layer-model (nu-mlm)
        # Original from Ahuja and Lehman, 1983 in @Shi2011
        b = 1  # [mm] Calibration constant, 1 >= b > 0 (b-ranges appear reasonable).
        # As b decreases, mass transfer increases, model.z0 in mm
        mass_ro = (precip * model.cell_area) * exp(-b * model.z0) * conc_layer_aq  # mg
        ...
        mass_ro = getKfilm(model, runoff_mm) * model.cell_area * conc_layer_aq  # mg
    else:
        print("Run-off transfer model not stated")
        return None
    return {"mass_runoff": mass_ro}
...
def getLeachedMass(model, layer, theta_sat,
    ...
    if leach_model == "mcgrath":
        if layer == 0:
            ...
            mass_layer = conc_layer_aq * (theta_layer * depth) * model.cell_area  # mg
            mass_layer_new = conc_layer_new_aq * (theta_after_percolate * depth) * model.cell_area  # mg
            mass_leached = mass_layer - mass_layer_new  # mg
        else:
            # McGrath not used in lower layers,
            # as formulation accounts for rainfall impact
            mass_leached = conc_layer_aq * tot_percolation * model.cell_area  # mg
