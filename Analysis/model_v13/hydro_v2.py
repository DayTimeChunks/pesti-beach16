...
def getLateralFlow(model, layer, run=True):
    depth = model.layer_depth[layer]
    c = model.c_lf[layer]
    isBasement = (layer == model.num_layers - 1)
    # Lateral Flow
    ################
    # PCRaster:
    ...
    if not run:
        ...
        if isBasement:
            f_pot = c * max(model.theta[layer], scalar(0))
        else:
            # All layers (except basement)
...
def getActualTransp(model, layer, root_depth_tot, root_depth, pot_transpir,
                    theta_wp, depletable_water, run=True):
    if run:
...
def getActualEvap(model, layer, theta_wp, pot_evapor, run=True):
