...
def getMassDegradation(model, layer, theta_wp, mass,
                       frac="L",
                       sor_deg_factor=1,
                       sorption_model="linear", fixed_dt50=True, biomass=False,
                       gas=True, debug=False, run=True):
    if not run:
        ...
        conc_ads = getConcAds(model, layer, mass, gas=gas)  # mass/g soil
        ...
        # Check gas
        mass_gas = mass - mass_aq - mass_ads
        if mapminimum(mass_gas) < 0:
            mass_gas = ifthenelse(mass_gas < 0, scalar(0), mass_gas)

        if biomass:
            bioa_mass = mass_ads * exp(-ln(2)/50)
            aged_mass = mass_ads - bioa_mass
        else:
            aged_mass = deepcopy(model.zero_map)

        # Degradation acts only on the reversible adsorbed fraction
        mass_ads -= aged_mass
        ...
        else:  # Same for total mass as for "H", but without alpha
    ...
    return {"mass_tot_new": mass_tot_new,  # Except aged fraction
