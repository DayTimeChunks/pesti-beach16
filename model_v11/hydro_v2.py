...
def runoff_SCS(model, rain,
    ...
    SFC = model.zero_map  # Soil water content at field capacity (mm)
    SS = model.zero_map  # Soil water content at saturation capacity (mm)
    SW = model.zero_map  # Soil Water content (mm)
    for layer in range(num_layers_scs):  # if == 2: z0 and z1
        ...
        SW += (model.theta[layer] - theta_wp) * model.layer_depth[layer]
...
def getPercolation(model, layer, k_sat, isPermeable=True):
    ...
    if layer < (len(model.layer_depth) - 1):  # layers: 0,1,2
        ...
        theta_check_bottom = model.theta[layer + 1] + percolation / model.layer_depth[layer + 1]
        satex_bottom = ifthenelse(theta_check_bottom > model.theta_sat[layer + 1],
                                  theta_check_bottom - model.theta_sat[layer + 1], scalar(0))
        percolation -= satex_bottom * model.layer_depth[layer + 1]  # mm
        percolation = max(scalar(0), percolation)
    else:  # Basement layer
        if not isPermeable:
            percolation = model.zero_map
...
def getLateralFlow(model, layer):
    depth = model.layer_depth[layer]
    c = model.c_lf[layer]
...
def getActualTransp(model, layer, root_depth_tot, root_depth, pot_transpir,
    ...
    pot_transpir_layer = ifthenelse(root_depth_tot > scalar(0),
                                    ...
                                    (root_depth / root_depth_tot) * pot_transpir,
