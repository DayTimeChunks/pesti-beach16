...
def getArtificialDrainage(model, layers):
    # Lateral Flow (artificial drainage)
    # now considers only one layer, modify if more needed.
    assert len(layers) == 1
    cell_drainge_outflow = max(model.c_adr * (model.layer_depth[layers[0]] * model.theta[layers[0]] -
                                              model.layer_depth[layers[0]] * model.theta_fc[layers[0]]),
                               scalar(0))  # [mm]
