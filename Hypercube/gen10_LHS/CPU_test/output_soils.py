...
def defineMassTSS(model, mMap="maps\\static\\outlet"):
    ...
    for i in range(len(names)):
        tss_name = names[i]
        model.catch_dict[tss_name] = TimeoutputTimeseries(names[i], model, nominal(mMap), noHeader=False, save_path="tss\\output\\" + str(model.period) + "\\")
...
def defineSoilTSS(model):
    ...
    for tr in range(len(transects)):
        ...
        model.soil_dict[transect_conc_real] = TimeoutputTimeseries("resM_" + transect_conc_real, model,
                                                                   ordinal(transect_map), noHeader=False, save_path="tss\\output\\" + str(model.period) + "\\")
    ...
    for plot in range(len(plots)):
        ...
        # Concentrations
        model.soil_dict[plot_conc] = TimeoutputTimeseries("resM_" + plot_conc, model, ordinal(plot_map), noHeader=False, save_path="tss\\output\\" + str(model.period) + "\\")
