...
class BeachModel(DynamicModel):
    ...
    Change above (#2) required change here of method report():
        line 64 to account for the new inpt parameter: 'directoryPrefix'
        line 64 to account a lisem run (default is False)

    """

    def __init__(self, cloneMap, names, params, upper, period,
                 end_days, end_days_remaining, event_type,
                 map_list, hydro_row, lisem_timestep,
                 staticDT50=False, correction=False, coupled=False # Used to include LISEM results in first time step.
                 ):
    ...
    def dynamic(self):
        ...
        for layer in range(self.num_layers):
            if layer == 0:  # Layer 0
                ...
                # infil_z1 is not added here because already added to the layer below, See above: SW1 (L:881)
                ...
                # RunOff Mass
                # TODO: Compute runoff-mass due to LISEM, update layer 0's masses!
                ...
                # TODO: Add runoff from LISEM
                runoff_m3 = runoff_z0 * cellarea() / 1000  # m3
                out_runoff_m3 = accuflux(self.ldd_subs, runoff_m3) + scalar(GA_runoff_m3)
                # out_runoff_m3 = areatotal(accu_runoff_m3, self.outlet) + scalar(GA_runoff_m3) # multi_outlet
        ...
        ###############################
        # LISEM and BEACHx input files deleted!
        ###############################
