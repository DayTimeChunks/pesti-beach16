...
class BeachModel(DynamicModel, MonteCarloModel):
    ...
    def premcloop(self):
        ...
        self.out_accu_latflow_m3_tss = TimeoutputTimeseries("res_accuLatflow_m3", self, nominal("outlet_true"),
                                                            noHeader=False)
        ...
        self.q_sim_tot_tss = TimeoutputTimeseries("res_q_sim_tot_m3", self, nominal("outlet_true"),
                                                  noHeader=False)  # This is 'Nash_q' as time series.
        self.outlet_mass_z0_tss = TimeoutputTimeseries("res_out_mass_sim_mg", self, nominal("outlet_true"),
                                                  noHeader=False)  # This is 'Nash_q' as time series.
        self.outlet_delta_z0_tss = TimeoutputTimeseries("res_out_delta_sim", self, nominal("outlet_true"),
                                                       noHeader=False)  # This is 'Nash_q' as time series.

        # Transects and detailed soils
        ###########
        self.obs_tr_soil_conc = TimeoutputTimeseries("res_tr_soil_conc", self, nominal("weekly_smp.map"),
                                                     noHeader=False)
        self.obs_tr_soil_mass = TimeoutputTimeseries("res_tr_soil_mass", self, nominal("weekly_smp.map"),
                                                     noHeader=False)
        self.obs_tr_soil_delta = TimeoutputTimeseries("res_tr_soil_delta", self, nominal("weekly_smp.map"),
                                                      noHeader=False)
        # self.obs_detail_soil = self.readmap("detailed_smp")
    ...
    def initial(self):
        ...
        # At variance = 5, will range btw. 5 and 35 approx (mean is 20)
        ...
        self.theta_z2 = max(readmap("ini_theta_z2"), scalar(0.01))
    ...
    def dynamic(self):
        ...
        if self.PEST:
            ...
            mass_ro = z0_mass_runoff["mass_runoff"]  # mg
            tot_massout_ro_z0 = accuflux(self.ldd_subs, mass_ro)
            deltaMass_ro = z0_mass_runoff["deltaMass_runoff"]
            tot_deltaMass_ro_z0 = accuflux(self.ldd_subs, deltaMass_ro)
            ...
            # Mass and deltaMass leaving the layer:
            tot_massout_latflux_z0 = accuflux(self.ldd_subs, z0_mass_latflux["net_mass_latflux"])
            tot_deltaMass_latflux_z0 = accuflux(self.ldd_subs, z0_mass_latflux["deltaMass_latflux"])
            ...
            # Outlet Mass and signature from layer z0
            outlet_mass_z0 = tot_massout_ro_z0 + tot_massout_latflux_z0
            outlet_delta_z0 = (tot_deltaMass_ro_z0 + tot_deltaMass_latflux_z0) / outlet_mass_z0
            self.outlet_mass_z0_tss.sample(outlet_mass_z0)
            self.outlet_delta_z0_tss.sample(outlet_delta_z0)
            # TODO: Latflow contribution to delta and mass to outlet from all other layers.
        ...
        if self.PEST:
            # Conc mg/L = mg * 1 / mm * 1/m2 * 10^3 mm/1m * 1m3/10^3L
            conc_z0 = self.pestmass_z0 / (self.theta_z0 * self.z0 * cellarea())  # mg/L
            self.obs_tr_soil_conc.sample(conc_z0)
            self.obs_tr_soil_mass.sample(self.pestmass_z0)
            self.obs_tr_soil_delta.sample(self.delta_z0)

            soil_comp = [182, 188, 197, 201, 209,  # April
                         216, 223, 230, 237, 244,  # May
                         251, 258, 265, 272,  # June
                         279, 286]  # July
            if self.currentTimeStep() in soil_comp:
                self.report(conc_z0, 'conc_z0')
                self.report(self.pestmass_z0, 'mass_z0')
                self.report(self.delta_z0, 'delta_z0')
        ...
        z2_moisture = getLayerMoisture(self, 2,
                                       ...
                                       percolate=percolation_z1, PERCOL_z2=PERCOL_z2)
        ...
        # Outflow is being subtracted from all cells,
        # thus LF at the outlet, needs to be = accuflux(cell_outflow)
        ...
        column_latflow_m3 = out_latflow_z0_m3 + out_latflow_z1_m3 + out_latflow_z2_m3
        out_latflow_m3 = accuflux(self.ldd_subs, column_latflow_m3)
        self.out_accu_latflow_m3_tss.sample(out_latflow_m3)

        # Net lateral flow
        net_latflow_z0_m3 = lat_flow_z0 * cellarea() / 1000  # m3
        net_latflow_z1_m3 = lat_flow_z1 * cellarea() / 1000
        net_latflow_z2_m3 = lat_flow_z2 * cellarea() / 1000
        ...
        self.q_sim_tot += ifthenelse(q_obs >= 0, vol_disch_m3, 0)
        self.q_diff += ifthenelse(q_obs >= 0, (vol_disch_m3 - q_obs) ** 2, 0)
