...
class BeachModel(DynamicModel, MonteCarloModel, ParticleFilterModel):
    def setDebug(self):
        pass
    ...
    def premcloop(self):
        ...
        self.out_latflow_m3_tss = TimeoutputTimeseries("res_outLatflow_m3", self, nominal("outlet_true"),
                                                       noHeader=False)
        self.out_net_latflow_m3_tss = TimeoutputTimeseries("res_accuLatflow_m3", self, nominal("outlet_true"),
                                                           noHeader=False)
        ...
        # Testing:
        self.out_vol_m3_tss = TimeoutputTimeseries("res_accuVol_m3", self, nominal("outlet_true"),
                                                   noHeader=False)  # This is 'q' but as time series.
    ...
    def initial(self):
        ...
        # mapnormal() -> the same for all catchment.
        self.s1 = mapnormal()*sqrt(10)  # 0 mean, 10 variance.  mm/day
        # self.s2 = self.ini_param.get("s2")  # coefficient to calibrate Ksat2

        # At variance = 5, mean will range btw. 5 and 35 approx (z0 mean is 20mm)
        self.z0 = self.zero_map
        self.z0 += 20 + mapnormal()*sqrt(25)  # mm
        self.z1 = self.zero_map + 300  # mm
        self.z2 = self.datum_depth + 610 - self.z0 - self.z1  # mm (300mm at outlet)
        ...
        # Non-stochastic:
        """ Physical parameters """
        self.c1 = self.ini_param.get("c1")  # subsurface flow coefficient
        self.c2 = self.ini_param.get("c2")  # not used (second layer)
        self.drain_coef = self.ini_param.get("drain_coef")  # drainage coefficient
        self.k = self.ini_param.get("k")  # coefficient of declining LAI in end stage

        """ Soil Properties """
        self.p_b = self.ini_param.get("p_b")  # Soil bulk density (g/cm^3)
        self.f_oc = self.ini_param.get("f_oc")  # Organic Carbon in soil without grass (kg/kg)
        ...
        self.k_oc = self.ini_param.get("k_oc")  # ml/g
        ...
        self.k_h = self.ini_param.get("k_h")  # Henry's constant @ 20 C (dimensionless, Metolachlor)
        ...
        self.dt_50_ref = self.ini_param.get("dt_50_ref")  # S-met (days)

        self.temp_ref = self.ini_param.get("temp_ref")  # Temp.  reference

        self.beta_moisture = self.ini_param.get("beta_moisture")  # Need to find a correct value for 'B', exponent in moisture dependency (Dairon)
        self.alpha_temperature = self.ini_param.get("alpha_temperature")  # Need to confirm units Ea = 54000 KJ/mol; R = 8.314 J/mol/Kelvin

        """
        Isotopes
        """
        self.r_standard = self.ini_param.get("r_standard")  # VPDB
        self.alpha_iso = self.ini_param.get("alpha_iso")  # 1 is no fractionation
    ...
    def dynamic(self):
        ...
        # adjusting K_sat (adds or subtracts stochastic variance)
        k_sat_z0z1 += self.s1
        self.report(k_sat_z0z1, 'ksatTop')  # Produces a map for every realization (& time-step relevant for later)
        # k_sat_z2 += self.s2
        ...
        if self.PEST:
            ...
            mass_applied = ifthenelse(self.jd_cum == 177,
                                      self.app1 * cellarea(),
                                      ifthenelse(self.jd_cum == 197, self.app2 * cellarea(),
                                                 ifthenelse(self.jd_cum == 238, self.app3 * cellarea(),
                                                            0)))  # [mg]
        ...
        # Runoff + accu_latflow
        self.vol_disch_m3 = out_runoff_m3 + out_net_latflow_m3
        self.out_vol_m3_tss.sample(self.vol_disch_m3)

        # Analysis (NASH Discharge)
        self.q_obs = timeinputscalar('q_obs_m3day.tss', nominal("outlet_true"))

        # Every time step a discharge map for all cells (accuflux)
        self.report(self.vol_disch_m3, 'q')
        # self.report(vol_tot_m3, 'storage')
        ...
        self.report(q_obs, 'q_obs')
        self.q_obs_tot += ifthenelse(q_obs >= 0, q_obs, 0)
        self.q_sim_tot += ifthenelse(q_obs >= 0, self.vol_disch_m3, 0)
        self.q_diff += ifthenelse(q_obs >= 0, (self.vol_disch_m3 - q_obs) ** 2, 0)
        ...
        # Visualization
        # aguila --scenarios='{1,2,3,4,5}' --multi=1x5  --timesteps=[170,280,1] ksatTop q outlet_true.map
        # aguila  1\res_accuStorage_m3.tss 2\res_accuStorage_m3.tss # shows only the non-dynamic one!
        # aguila  2\res_accuEtp_m3.tss 2\res_accuStorage_m3.tss 2\res_accuChStorage_m3.tss
    ...
    def updateWeight(self):
        modelledData = self.readmap('q')
        outlet = readmap("outlet_true")
        out = boolean(outlet)
        modelledData = ifthenelse(out, modelledData, scalar(0))
        modelledAveMap = areaaverage(modelledData, outlet)
        observedAveMap = self.readDeterministic('q_obs')
        observedStdDevMap = ifthenelse(observedAveMap > 0, scalar(sqrt(260.07)), 0.01)
        sum_map = maptotal(((observedAveMap - modelledAveMap) ** 2) / (-2.0 * (observedStdDevMap ** 2)))


        modelledAveMap =
        sum_t = (self.q_diff)/(-2.0*self.q_var)
        weight = exp(sum_t)
        assert type(weight) == float, "Error with the error sum: %d" % weight
        return float(weight)

    def suspend(self):
        self.reportState(self.vol_disch_m3, 'q')




# aguila --scenarios='{1,2,3,4,5}' --multi=1x5 --timesteps=[170,280,1] thetaZ q storage z0_depth outlet_true

nrOfSamples = 5  # Samples are each a MonteCarlo realization
firstTimeStep = 170
nTimeSteps = 280
...
# Currently evaluating two stochastic parameters:
# ksatTop, adjusted via s1  (not having much effect with var = 10)
# z0_depth, var = 25 (Ho: should not have effect due to SCS two-layer adaptation)
#   -> Small effect but difficult to evaluate without criteria. Integrate to model_v4 tests, if run appears promising.

# Next step: Introduce Paul's Ksat, which is about 5 times larger! Not done...
#
