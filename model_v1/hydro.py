...
def runoff_SCS(model, rain,
    ...
    CN2 = ifthenelse(crop_type > scalar(1), CN2,
                     ifthenelse(crop_type == scalar(0), CN2,
                                ifthenelse(jd_sim < jd_dev, scalar(85),
                                           ifthenelse(jd_sim <= jd_mid,
                                                      CN2 + (scalar(85) - CN2) * ((jd_mid - jd_sim) / len_dev_stage),
                                                      ifthenelse(jd_sim <= jd_end, CN2, scalar(85))))))
...
def getLayerMoisture(model, layer,
    ...
    tau = 0.0866 * exp(model.drain_coef * log10(k_sat))  # dimensionless drainage param.
    ...
    if layer == 0:
        ...
        theta_temp_layer -= deep_percolation / depth
        theta_after_percolate = theta_temp_layer
        water_balance_layer = infil - deep_percolation  # TODO: adjust if cannot percolate to z > 2
    ...
    # Cell outflow
    cell_moisture_outflow = max(model.c1 * (depth * theta_temp_layer - depth * theta_fcap), scalar(0))  # [mm]
