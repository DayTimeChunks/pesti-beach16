    ...
    if not run:
        ...
        if rel_diff_model == "option-1":
            ...
            diff_relative_gas = (diff_a * theta_gas ** 2 /
                                 model.theta_sat[layer] ** (2 / 3))  # m2/d
    ...
    if not run:
        ...
        if leach_model == "mcgrath":
            if layer == 0:
                ...
                if mapminimum(mass_aq_new) < -1*10e-06:
                    print("Error in Leached Model, layer: ", str(layer))
                    model.report(mass_leached, 'aZ' + str(layer) + 'LCH')
...
def getLatMassFlux(model, layer, mass, cell_moisture_outflow, upstream_cell_inflow,
    ...
    if not run:
        ...
        mass_loss = conc_aq * cell_moisture_outflow
        mass_gain = upstream(model.ldd_subs, conc_aq * upstream_cell_inflow)
...
def getMassDegradation(model, layer, theta_wp, mass,
                       ...
                       gas=True, debug=False, run=True):
    if not run:
        return {"mass_tot_new": model.zero_map,
