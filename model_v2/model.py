...
class BeachModel(DynamicModel):
    ...
    def initial(self):
        ...
        with open(ini_path, 'r') as f:
            ...
            for row in reader:
                ini_param[row[0].strip()] = float(row[1])

        """ Physical parameters """
        self.c1 = ini_param.get("c1")  # subsurface flow coefficient
        self.c2 = ini_param.get("c2")  # not used (second layer)
        self.drain_coef = ini_param.get("drain_coef")  # drainage coefficient
        self.s1 = ini_param.get("s1")  # coefficient to calibrate Ksat1
        self.s2 = ini_param.get("s2")  # coefficient to calibrate Ksat2
        self.k = ini_param.get("k")  # coefficient of declining LAI in end stage

        """ Soil Properties """
        self.p_b = ini_param.get("p_b")  # Soil bulk density (g/cm^3)
        self.f_oc = ini_param.get("f_oc")  # Organic Carbon in soil without grass (kg/kg)
        ...
        self.k_oc = ini_param.get("k_oc")  # ml/g
        ...
        self.k_h = ini_param.get("k_h")  # Henry's constant @ 20 C (dimensionless, Metolachlor)
        ...
        self.dt_50_ref = ini_param.get("dt_50_ref")  # S-met (days)
        ...
        Isotopes
        """
        self.r_standard = ini_param.get("r_standard")  # VPDB
        self.alpha_iso = ini_param.get("alpha_iso")  # 1 is no fractionation
    ...
    def dynamic(self):
        ...
        if timeStep == 274:  # June 30
            ...
            report(self.theta_z0, "resV2" + str(timeStep) + "_theta_z0.map")  # Check ditch results
            report(self.theta_z1, "resV2" + str(timeStep) + "_theta_z1.map")  # Check ditch results
            report(self.theta_z2, "resV2" + str(timeStep) + "_theta_z2.map")  # Check ditch results
            # report(nmap, 'zzTest.map')
            # aguila(nmap, self.temp_z0_fin)
            print 'dynamic time step: ', timeStep
