...
class BeachModel(DynamicModel, MonteCarloModel):
    ...
    def initial(self):
        ...
        if m_state == 0:
            ...
            epsilon_iso = scalar(self.ini_param.get("epsilon_iso_ref"))  # 2 is no fractionation, -2.743 -> low deg
            self.alpha_iso = epsilon_iso / 1000 + 1
            # for layer in range(self.num_layers):
            ...
            epsilon_iso = scalar(self.ini_param.get("epsilon_iso_max"))  # 2 is no fractionation, -2.743 -> low deg
            self.alpha_iso = epsilon_iso / 1000 + 1
            # for layer in range(self.num_layers):
            #     self.c_lf[layer] = 2
